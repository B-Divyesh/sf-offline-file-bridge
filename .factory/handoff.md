...
- **Repaired release:** `v0.1.6` / Android version code `6`
...
This repair increments both web and Android versions to `0.1.6`/`6`. The
...
POSIX `/bin/sh`, so the wait block deliberately uses `set -eu` rather than the
unsupported `pipefail` option; this exact compatibility condition is also
regression-tested.
...
It downloads the public `v0.1.6` APK and verifies every embedded web payload
